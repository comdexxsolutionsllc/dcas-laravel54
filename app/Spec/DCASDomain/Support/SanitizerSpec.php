...
class SanitizerSpec extends ObjectBehavior {
    ...
    function let()
    {
        $this->beAnInstanceOf('Spec\DCASDomain\Support\TestSanitizer');
    }


    function it_sanitizes_data_against_a_set_of_rules()
    {
        $this->sanitize([ 'slug' => 'SOME-SLUG' ], [ 'slug' => 'strtolower' ])->shouldReturn([ 'slug' => 'some-slug' ]);

        $this->sanitize([ 'first' => 'john' ],
            [ 'first' => 'ucwords', 'last' => 'ucwords' ])->shouldReturn([ 'first' => 'John' ]);
    }


    function it_cen_apply_multiple_santizers()
    {
        $this->sanitize([ 'name' => '   john doe  ' ],
            [ 'name' => 'trim|ucwords' ])->shouldReturn([ 'name' => 'John Doe' ]);
    }
